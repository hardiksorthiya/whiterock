@php
    ...
    $rightSliderImages = collect($rightSliderImages ?? [])
        ->filter()
        ->values();
                    ...
                    <div class="home-category-showcase__media">
                        @if ($rightSliderImages->isNotEmpty())
                            <div id="home-right-slider-{{ $slugPrefix }}"
                                class="carousel slide home-category-showcase__right-slider" data-bs-ride="carousel"
                                data-bs-interval="2600" data-bs-pause="false" data-bs-touch="true" data-bs-wrap="true">
                                <div class="carousel-inner">
                                    @foreach ($rightSliderImages as $sliderImage)
                                        <div class="carousel-item @if ($loop->first) active @endif">
                                            <img src="{{ $sliderImage }}" alt="{{ $sectionTitle }}"
                        ...
                        @else
                            <img src="{{ $imageUrl }}" alt="{{ $sectionTitle }}" class="img-fluid w-100 h-100">
                        @endif
                    </div>
                </div>
                ...
                <div class="col-lg-6 d-flex">
                    <div class="home-category-showcase__content w-100 d-flex flex-column justify-content-center">
                        @if (!empty($titleImageUrl))
                            <img src="{{ $titleImageUrl }}" alt="{{ $sectionTitle }}"
