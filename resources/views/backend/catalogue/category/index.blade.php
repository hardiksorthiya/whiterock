...
@section('page_title', 'Catalogue categories')
@section('page_subtitle', 'Organize catalogues by category')

@section('content')
@php($bulkFormId = 'catalogue-category-bulk-form')


    <div class="adm-toolbar d-flex flex-wrap align-items-end gap-3 justify-content-between">
        <a href="{{ route('backend.catalogue-categories.create') }}" class="adm-btn adm-btn--primary">
            <i class="bi bi-plus-lg"></i> Add category
        </a>

        <form method="GET" action="{{ route('backend.catalogue-categories.index') }}"
            ...
            @if (request()->filled('search'))
                <a href="{{ route('backend.catalogue-categories.index') }}" class="adm-btn adm-btn--ghost adm-btn--sm">Clear</a>
            @endif
        </form>
    </div>
        ...
        @if ($categories->isEmpty())
            <p class="adm-empty">
                @if (request()->filled('search'))
                    ...
                    <a href="{{ route('backend.catalogue-categories.index') }}">Clear search</a>
                @else
                    No categories yet. <a href="{{ route('backend.catalogue-categories.create') }}">Create the first one</a>.
                @endif
            </p>
        @else
            <form id="{{ $bulkFormId }}" method="POST" action="{{ route('backend.catalogue-categories.bulk-action') }}"
                            ...
                            <tr>
                                <td>
                                    <input type="checkbox" class="select-item" name="category_ids[]"
                                        value="{{ $category->id }}" form="{{ $bulkFormId }}">
                                </td>
                                <td><span
                                        class="adm-badge">{{ $loop->iteration + ($categories->currentPage() - 1) * $categories->perPage() }}</span>
                                </td>
                                <td>
                                    <strong>{{ $category->name }}</strong></td>
                                <td><span class="adm-muted small">{{ $category->slug }}</span></td>
                            
                                <td class="text-end">
                                    <div class="adm-actions justify-content-end">
                                        <a href="{{ route('backend.catalogue-categories.edit', $category) }}"
                                            class="adm-btn adm-btn--ghost adm-btn--sm">Edit</a>
                                        <form action="{{ route('backend.catalogue-categories.destroy', $category) }}"
...
(function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (!n) {
            e.preventDefault();
            alert('Select at least one catalogue category.');
            return;
        }
        if (action.value === 'delete' && !confirm('Delete ' + n + ' categor' + (n === 1 ? 'y' : 'ies') + '? Linked catalogues may be deleted if the database is set to cascade.')) {
            e.preventDefault();
            }
        });
    })();
